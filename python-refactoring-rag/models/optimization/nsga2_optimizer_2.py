...
class ImprovedModelOptimizationProblem(Problem):
    ...
    def __init__(self, evaluation_results: Dict[str, List[Dict]]):
        ...
        # n_models+2 to n_models+5: Metric importance weights (4 weights)
        
        n_vars = self.n_models + 2 + 4  # model_weights + retrieval_params + metric_weights
        ...
        super().__init__(
            ...
            xu=np.array([1.0] * self.n_models + [0.8, 3.0] + [1.0] * 4),  # Upper bounds
        )
    ...
    def _calculate_comprehensive_model_metrics(self) -> Dict[str, Dict[str, float]]:
        ...
        for model_name, results in self.evaluation_results.items():
            ...
            for r in successful_results:
                ...
                metrics_data['codebleu_score'].append(self._safe_metric(getattr(rag, 'codebleu_score', 0.01)))
                metrics_data['latency_ms'].append(self._safe_metric(rag.latency_ms, default=1000, min_val=0, max_val=30000))
                
                # Handle token usage
                token_usage = getattr(rag, 'token_usage', {})
                total_tokens = token_usage.get('total_tokens', 0) if isinstance(token_usage, dict) else 0
                metrics_data['total_tokens'].append(self._safe_metric(total_tokens, default=100, min_val=0, max_val=50000))
            ...
            for metric_name, values in metrics_data.items():
                if values:
                    ...
                    metrics[f'{metric_name}_mean'] = default_val
                    metrics[f'{metric_name}_std'] = 0.0
                    metrics[f'{metric_name}_median'] = default_val
                    metrics[f'{metric_name}_q75'] = default_val
                    metrics[f'{metric_name}_robustness'] = 0.1
            
            # Overall performance aggregates
            core_metrics = ['answer_relevance', 'faithfulness', 'response_completeness']
            code_metrics = ['codebleu_score']
            efficiency_metrics = ['latency_ms', 'total_tokens']
            
            # Weighted overall performance
            core_score = np.mean([metrics[f'{m}_mean'] for m in core_metrics])
            code_score = np.mean([metrics[f'{m}_mean'] for m in code_metrics])
            
            # Efficiency score (lower latency and tokens is better, so invert)
            latency_score = 1.0 / (1.0 + metrics['latency_ms_mean'] / 1000.0)  # Normalize latency
            token_score = 1.0 / (1.0 + metrics['total_tokens_mean'] / 1000.0)  # Normalize tokens
            efficiency_score = (latency_score + token_score) / 2
            
            metrics['overall_performance'] = 0.6 * core_score + 0.2 * code_score + 0.2 * efficiency_score
            
            # Consistency score (average robustness)
            all_metrics = core_metrics + code_metrics
            metrics['overall_consistency'] = np.mean([metrics[f'{m}_robustness'] for m in all_metrics])
            ...
            # Efficiency (combination of latency and token efficiency)
    ...
    def _get_default_metrics(self) -> Dict[str, float]:
        ...
        base_metrics = ['answer_relevance', 'faithfulness', 
                    'response_completeness', 'codebleu_score',
        ...
        for metric in base_metrics:
            ...
            defaults[f'{metric}_mean'] = default_val
            defaults[f'{metric}_std'] = 0.05
            defaults[f'{metric}_median'] = default_val
            defaults[f'{metric}_q75'] = default_q75
        ...
        try:
            ...
            for i, solution in enumerate(X):
                try:
                    ...
                    metric_weights_raw = solution[self.n_models + 2:self.n_models + 6]
...
class FixedNSGA2ModelSelector:
    ...
    def _validate_model_diversity(self):
        ...
        for model_name, results in self.evaluation_results.items():
            ...
            if successful_results:
                ...
                for r in successful_results:
                    rag = r['rag_metrics']
                    score = (rag.answer_relevance + 
                            rag.faithfulness + rag.response_completeness) / 3.0
                    scores.append(score)
    ...
    def _process_improved_results(self, res, optimization_time: float, n_generations: int) -> Dict:
        ...
        try:
            ...
            metric_weights_raw = best_solution_vars[self.problem.n_models + 2:self.problem.n_models + 6]
            ...
            return {
                ...
                'optimized_weights': {
                    ...
                    'answer_relevance': float(metric_weights[0]),
                    'faithfulness': float(metric_weights[1]),
                    'response_completeness': float(metric_weights[2]),
                    'codebleu_score': float(metric_weights[3])
                },
                'optimized_retrieval': {
    ...
    def _calculate_baseline_metrics(self) -> Dict[str, float]:
        ...
        metric_weights = np.ones(4) / 4  # Updated for 4 metrics 
    ...
    def _calculate_optimized_metrics(self, model_weights: np.ndarray, 
        ...
        for metric in base_metrics + efficiency_metrics:
            ensemble_value = 0.0
            for i, model_name in enumerate(self.problem.model_names):
                model_metrics = self.problem.model_metrics[model_name]
                model_value = model_metrics.get(f'{metric}_mean', 0.1 if metric not in efficiency_metrics else (1000 if metric == 'latency_ms' else 100))
    ...
    def _calculate_improvement_metrics(self, baseline: Dict[str, float], 
        ...
        quality_metrics = ['answer_relevance', 'faithfulness',
                        'response_completeness', 'codebleu_score']
        
        # Efficiency metrics (lower is better)
        efficiency_metrics = ['latency_ms', 'total_tokens']
        
        # Calculate improvements for quality metrics
        ...
        # Calculate changes for efficiency metrics (note: lower is better, so improvements are negative changes)
        for metric in efficiency_metrics:
            baseline_val = baseline.get(metric, 1000 if metric == 'latency_ms' else 100)
            optimized_val = optimized.get(metric, 1000 if metric == 'latency_ms' else 100)
            
            # For efficiency metrics, a decrease is an improvement
            absolute_change = optimized_val - baseline_val
            relative_change = absolute_change / baseline_val if baseline_val > 0 else 0.0
            
            improvements[f'{metric}_absolute_change'] = absolute_change
            improvements[f'{metric}_relative_change'] = relative_change
            
            # For reporting purposes, improvement is negative change (reduction)
            improvements[f'{metric}_improvement'] = -absolute_change
            improvements[f'{metric}_relative_improvement'] = -relative_change
        
        # Overall improvement (quality metrics only)
        quality_baseline = np.mean([baseline.get(m, 0.1) for m in quality_metrics])
        quality_optimized = np.mean([optimized.get(m, 0.1) for m in quality_metrics])
        
        if quality_baseline > 0:
            improvements['overall_relative_improvement'] = (quality_optimized - quality_baseline) / quality_baseline
        else:
            improvements['overall_relative_improvement'] = 0.0
        
        improvements['overall_absolute_improvement'] = quality_optimized - quality_baseline
    ...
    def _single_model_result(self) -> Dict:
        ...
        single_model_metrics = {
            ...
            'codebleu_score': metrics.get('codebleu_score_mean', 0.01)
        }
    ...
    def _intelligent_fallback_selection(self) -> Dict:
        ...
        try:
            ...
            baseline_metrics = {
                ...
                'codebleu_score': model_metrics.get('codebleu_score_mean', 0.01) * 0.95
            }
            ...
            optimized_metrics = {
                ...
                'codebleu_score': model_metrics.get('codebleu_score_mean', 0.01)
            }
