...
try:
    from services.interactive_display_manager import create_interactive_session
    ...
    for model_name in available_models:
        ...
        for i, test_case in enumerate(test_cases):
            ...
            try:
                ...
                # Get token usage (if available from system)
                token_usage = getattr(system.llm_provider, 'last_token_usage', None)
                if token_usage is None:
                    # Estimate token usage based on text length (more accurate estimation)
                    input_tokens = len(test_case['query'].split()) * 1.3  # Rough estimation
                    output_tokens = len(suggestion.split()) * 1.3
                    token_usage = {
                ...
                if result['success'] and result['rag_metrics']:
                    ...
                    tokens = rag.token_usage.get('total_tokens', 0) if hasattr(rag, 'token_usage') and rag.token_usage else 0
                    print(f"   Case {i+1}: Answer:{rag.answer_relevance:.3f} | "
                          f"Faithful:{rag.faithfulness:.3f} | "
...
def display_evaluation_summary(evaluation_results: Dict[str, Any]):
    ...
    for model_name, results in evaluation_results.items():
        ...
        if successful_results:
            ...
            avg_codebleu = sum(r['rag_metrics'].codebleu_score for r in successful_results) / len(successful_results)
            avg_latency = sum(r['rag_metrics'].latency_ms for r in successful_results) / len(successful_results)
            
            # Calculate average token usage
            avg_tokens = 0
            token_count = 0
            for r in successful_results:
                token_usage = getattr(r['rag_metrics'], 'token_usage', {})
                if isinstance(token_usage, dict) and 'total_tokens' in token_usage:
                    avg_tokens += token_usage['total_tokens']
                    token_count += 1
            avg_tokens = avg_tokens / max(1, token_count)
            
            model_aggregates[model_name] = {
                'answer_relevance': avg_answer_rel,
                'faithfulness': avg_faithfulness,
                'response_completeness': avg_completeness,
                'codebleu_score': avg_codebleu,
    ...
    for model_name, metrics in model_aggregates.items():
        print(f"{model_name:<25} "
              f"{metrics['answer_relevance']:<7.3f} "
              f"{metrics['faithfulness']:<7.3f} "
              f"{metrics['response_completeness']:<8.3f} "
              f"{metrics['codebleu_score']:<8.4f} "
              f"{metrics['latency_ms']:<8.0f} "
              f"{metrics['avg_tokens']:<7.0f} "
              f"{metrics['success_rate']:<7.2%}")
...
def run_model_optimization(evaluation_results: Dict[str, Any], 
    ...
    try:
        ...
        if baseline_metrics and optimized_metrics:
            ...
            print(f"\n{'CORE QUALITY METRICS (Higher is Better)':<60}")
            print(f"{'='*65}")
            print(f"{'Metric':<25} {'Before':<10} {'After':<10} {'Change':<12} {'% Change':<10}")
            print("-" * 65)
            
            core_metrics = ['answer_relevance', 'faithfulness', 
            ...
            for metric in core_metrics:
                if metric in baseline_metrics and metric in optimized_metrics:
                    ...
                    core_improvements.append(rel_change)
                    ...
                    # Color coding for significant improvements
                    ...
                    print(f"{metric.replace('_', ' ').title():<25} "
                          ...
                          f"{after_val:<10.4f} "
                          f"{change_str:<12} "
                          f"{percent_str:<10}{improvement_indicator}")
            
            # Calculate core metrics summary
            avg_core_improvement = np.mean(core_improvements) if core_improvements else 0.0
            print("-" * 65)
            print(f"{'CORE METRICS AVERAGE':<25} {'':10} {'':10} {'':12} "
                  f"{'+' if avg_core_improvement >= 0 else ''}{avg_core_improvement*100:.2f}%")
            
            # EFFICIENCY METRICS
            efficiency_metrics = []
            if 'latency_ms' in baseline_metrics and 'latency_ms' in optimized_metrics:
                efficiency_metrics.append('latency_ms')
            if 'total_tokens' in baseline_metrics and 'total_tokens' in optimized_metrics:
                efficiency_metrics.append('total_tokens')
            
            if efficiency_metrics:
                print(f"\n{'EFFICIENCY METRICS (Lower is Better)':<60}")
                print(f"{'='*65}")
                print(f"{'Metric':<25} {'Before':<10} {'After':<10} {'Change':<12} {'% Change':<10}")
                print("-" * 65)
                ...
                for metric in efficiency_metrics:
                    before_val = baseline_metrics[metric]
                    after_val = optimized_metrics[metric]
                    abs_change = after_val - before_val
                    rel_change = abs_change / before_val if before_val > 0 else 0.0
                    
                    # For efficiency, improvement is negative change (reduction)
                    efficiency_improvement = -rel_change
                    efficiency_improvements.append(efficiency_improvement)
                    
                    change_str = f"+{abs_change:.1f}" if abs_change >= 0 else f"{abs_change:.1f}"
                    percent_str = f"+{rel_change*100:.1f}%" if rel_change >= 0 else f"{rel_change*100:.1f}%"
                    
                    # Improvement indicators for efficiency (lower is better)
                    improvement_indicator = ""
                    if rel_change < -0.10:  # >10% reduction
                        improvement_indicator = ""
                    elif rel_change < -0.05:  # >5% reduction
                        improvement_indicator = ""
                    elif rel_change > 0.05:  # >5% increase (worse)
                        improvement_indicator = ""
                    
                    unit = "ms" if metric == 'latency_ms' else "tokens"
                    print(f"{metric.replace('_', ' ').title():<25} "
                          f"{before_val:<10.1f} "
                          f"{after_val:<10.1f} "
                          f"{change_str:<12} "
                ...
                avg_efficiency_improvement = np.mean(efficiency_improvements) if efficiency_improvements else 0.0
                print("-" * 65)
                print(f"{'EFFICIENCY AVERAGE':<25} {'':10} {'':10} {'':12} "
...
def display_optimization_summary_enhanced(optimization_results: Dict[str, Any]):
    ...
    if 'optimized_retrieval' in optimization_results:
        ...
        print(f"2. Optimize retrieval settings:")
        print(f"   - Similarity threshold: {threshold:.3f}")
        print(f"   - Top-K multiplier: {topk:.2f}")
    
    # Performance insights
    best_metrics = ['answer_relevance', 'faithfulness', 'response_completeness', 'codebleu_score']
    best_improvements = []
    ...
    if best_improvements:
        ...
        print(f"3. Strongest improvements in:")
    ...
    efficiency_insights = []
    if 'latency_ms' in baseline_metrics and 'latency_ms' in optimized_metrics:
        latency_change = optimized_metrics['latency_ms'] - baseline_metrics['latency_ms']
        if abs(latency_change) > 100:  # Significant change (>100ms)
            if latency_change < 0:
                efficiency_insights.append(f"Reduced latency by {abs(latency_change):.0f}ms")
            else:
                efficiency_insights.append(f"Increased latency by {latency_change:.0f}ms (trade-off for quality)")
    
    if 'total_tokens' in baseline_metrics and 'total_tokens' in optimized_metrics:
        token_change = optimized_metrics['total_tokens'] - baseline_metrics['total_tokens']
        if abs(token_change) > 50:  # Significant change (>50 tokens)
            if token_change < 0:
                efficiency_insights.append(f"Reduced token usage by {abs(token_change):.0f} tokens")
            else:
                efficiency_insights.append(f"Increased token usage by {token_change:.0f} tokens (trade-off for quality)")
    
    if efficiency_insights:
        print(f"4. Efficiency changes:")
...
def main():
    ...
    PDF_PATHS = [
        ...
        "Inputs\\Expert_Knowledge\\the-clean-coder-a-code-of-conduct-for-professional-programmers.pdf" 
    ]
